...
Available only on `net11.0` with `<LangVersion>preview</LangVersion>`. These back the C# 15 union `switch`/`is` matching — see [Union matching](./#union-matching-net11--c-15).

| Member | Signature | Description |
| --- | --- | --- |
| `Value` | `object? Value { get; }` | The value when Some, `null` when None. **Boxes** a value-type Some — prefer typed `switch`/`is` arms. |
| `HasValue` | `bool HasValue { get; }` | `true` when Some (equivalent to `IsSome`). |
| `TryGetValue` | `bool TryGetValue(out T? value)` | Non-boxing union access: `true` and the value when Some. Used by the compiler for value-type `switch` arms. |
| `IUnionMembers.Create` | `static Maybe<T> Create(T value)` | Union creation member; delegates to `Maybe.Some`. |
