...
public static partial class ResultExtensions
{
    ...
    extension<T1>(Result<T1> result) where T1 : notnull
    {
        ...
        /// </returns>
        public async ValueTask<Result<T1>> ThenAsync(Func<T1, ValueTask<Result>> then,
            bool copyReasonsAndMetadata = true)
        {
            if (!result.TryGet(out T1? value)) return result;
    ...
    extension<TValue>(ValueTask<Result<TValue>> awaitableResult) where TValue : notnull
    {
        ...
        /// <param name="copyReasonsAndMetadata">Whether to copy reasons and metadata from original result.</param>
        /// <returns>A task with the new result.</returns>
        public async ValueTask<Result<TValue>> ThenAsync(Func<TValue, Result<TValue>> then,
            bool copyReasonsAndMetadata = true)
        {
        ...
        public async ValueTask<Result<TValue>> ThenAsync(
        ...
        /// <param name="copyReasonsAndMetadata">Whether to copy reasons and metadata from original result.</param>
        /// <returns>A task with the new result.</returns>
        public async ValueTask<Result<TValue>> ThenAsync(Func<TValue, ValueTask<Result<TValue>>> then,
            bool copyReasonsAndMetadata = true)
        {
            var result = await awaitableResult.ConfigureAwait(false);
            return await result.ThenAsync(then, copyReasonsAndMetadata).ConfigureAwait(false);
