...
public static class MaybeAssertionExtensions
{
    ...
    ///     Some result of the original <see cref="MaybeAssertion{TValue}" />.
    /// </returns>
    ...
    ///     <see cref="Maybe{TValue}" /> result.
    /// </returns>
    public static async Task<MaybeAssertion<TValue>> ShouldBe<TValue>(this Task<Maybe<TValue>> awaitableMaybe)
        where TValue : notnull
    {
        var maybe = await awaitableMaybe;
        return new MaybeAssertion<TValue>(maybe);
    ...
    ///     Some result of the original <see cref="MaybeAssertion{TValue}" />.
    /// </returns>
    public static async Task<SomeAssertion<TValue>> Some<TValue>(this Task<MaybeAssertion<TValue>> awaitableAssertion)
        where TValue : notnull
    {
        var assertion = await awaitableAssertion;
        return assertion.Some();
    ...
    ///     of the provided <see cref="MaybeAssertion{TValue}" />.
    /// </returns>
    public static async Task<NoneAssertion> None<TValue>(this Task<MaybeAssertion<TValue>> awaitableAssertion)
        where TValue : notnull
    {
        var assertion = await awaitableAssertion;
        return assertion.None();
    ...
    ///     post-assertion, allowing for further chaining or transformations.
    /// </returns>
    public static async Task<SomeAssertion<TValue>> And<TValue>(this Task<SomeAssertion<TValue>> awaitableAssertion,
                                                                Action<TValue>                    assert)
        where TValue : notnull
    {
        var assertion = await awaitableAssertion;
    ...
    ///     Thrown if the <paramref name="map" /> function is null.
    /// </exception>
    public static async Task<SomeAssertion<TOut>> Map<TValue, TOut>(this Task<SomeAssertion<TValue>> awaitableAssertion,
                                                                    Func<TValue, TOut>                map)
        where TValue : notnull
        where TOut : notnull
    {
