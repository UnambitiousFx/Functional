...
public static class MaybeHttpExtensions
{
    ...
    ///     <see cref="IActionResult" />.
    /// </summary>
    public static MaybeMvcBuilder<TValue> AsActionResultBuilder<TValue>(this Task<Maybe<TValue>> maybeTask)
        where TValue : notnull
    {
        return new MaybeMvcBuilder<TValue>(new ValueTask<Maybe<TValue>>(maybeTask));
    }
}
