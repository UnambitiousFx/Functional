...
public static class MaybeHttpExtensions
{
    ...
    /// <typeparam name="TValue">The type of the contained value.</typeparam>
    /// <returns>A <see cref="MaybeHttpBuilder{TValue}" /> for configuring the HTTP response.</returns>
    public static MaybeHttpBuilder<TValue> AsHttpBuilder<TValue>(this Task<Maybe<TValue>> maybeTask)
        where TValue : notnull
    {
        return new MaybeHttpBuilder<TValue>(new ValueTask<Maybe<TValue>>(maybeTask));
    }
}
