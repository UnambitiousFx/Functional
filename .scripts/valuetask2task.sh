...
plan_actions_for_path() {
  ...
  echo
  echo "  Add C# auto-generated header to .cs files (if not already present):"
  find "$SRC" -type f -name '*.cs' -print | sed -e "s|^$SRC/||" | sed 's/^/    /' || true
  echo
...
process_path() {
  ...
  # Add C# auto-generated header to .cs files (skip when already present)
  echo "Adding C# auto-generated header to new .cs files..."
  find "$DEST" -type f -name '*.cs' -print | while IFS= read -r csfile; do
    # Check first 10 lines for the standard auto-generated tag
    if ! sed -n '1,10p' "$csfile" | grep -q '<auto-generated>'; then
      echo "  Adding header to: $csfile"
      tmpfile="$csfile.header.tmp"
      cat > "$tmpfile" <<EOF
// <auto-generated>
...
EOF
      # Prepend header and replace original
      cat "$csfile" >> "$tmpfile" && mv "$tmpfile" "$csfile" || { rm -f "$tmpfile"; echo "  Failed to add header to: $csfile" >&2; }
    else
      echo "  Skipping header for (already generated): $csfile"
    fi
  done
