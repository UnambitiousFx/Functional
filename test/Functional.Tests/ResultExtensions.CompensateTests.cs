...
public class ResultExtensionsCompensateTests
{
    ...
    {
        // Arrange (Given)
        var originalError = new Failure(originalMsg);
        var rollbackError = new Failure(rollbackMsg);
        var result        = Result.Failure(originalError);
        ...
        compensated.ShouldBe()
                   ...
                   .And(error =>
                    {
                        var aggregateError = Assert.IsType<AggregateFailure>(error);
                        Assert.Equal(originalMsg, aggregateError.Errors.ElementAt(0)
                                                                .Message);
                        Assert.Equal(rollbackMsg, aggregateError.Errors.ElementAt(1)
                                                                .Message);
                    });
    }
